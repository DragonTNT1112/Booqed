...
class MainWindow():
    ...
    def connect_to_pod(self):
        ...
        if len(self.ID_Input.get("1.0", "end-1c")) != 0:
            try:
                temp_ID = int(self.ID_Input.get("1.0", "end-1c"))
                self.log_box.config(state="normal")
                self.log_box.insert("end", "{} - Trying to connect to Pod ({})...\n".format(time, temp_ID))
                self.log_box.config(state="disabled")

                self.client.publish(MQTT_TOPIC, temp_ID)
            except:
                self.log_box.config(state="normal")
    ...
    def get_qubic_status(self):
        time = str(datetime.now())[:-7]
        MQTT_TOPIC = "Qubic/Get Pod Status"
    ...
    def get_resent_data(self):
        time = str(datetime.now())[:-7]
        MQTT_TOPIC = "Qubic/Get Un-sent Images"
        self.client.publish(MQTT_TOPIC, 1)
        self.log_box.config(state="normal")
    ...
    def on_message(self, client, userdata, message):
        if message.topic == "Qubic/Connection State":
            if message != None:
                time = str(datetime.now())[:-7]
        ...
        if message.topic == "Qubic/Pod Status":
        ...
        if message.topic == "Qubic/Requested Images":
            # more callbacks, etc
            ...
            directory = "C:/Users/zzh84/OneDrive/Documents/GitHub/Booqed/Received_images/{}/{}/{}/".format(self.ID,
                                                                                                           date_folder,
                                                                                                           time_period)
        ...
        if message.topic == "Qubic/Differences":
            time = str(datetime.now())[:-7]
        ...
        if message.topic == "Qubic/Sync Database":
            time = str(datetime.now())[:-7]
